...
def sales_mart_calculation_table_write(final_sales_team_data_mart_df):
    ...
    final_sales_data_mart_table = final_sales_team_data_mart \
    ...
    print("Writing into sales team data mart")
    db_writer = DatabaseWriter(url=config.url, properties=config.properties)
    db_writer.write_dataframe(final_sales_data_mart_table, config.sales_team_data_mart_table)
